		...
		>
			...
			<div className="mx-auto max-w-5xl relative z-10">
							...
							<FadeInItem key={p.title} yOffset={24} className="h-full">
								<button
									...
									className={`w-full text-left relative group bg-surface-night p-8 rounded-xl border flex flex-col justify-between h-full hover:-translate-y-1 hover:border-accent-lime/40 transition-all duration-300 ${
										isSelected
											? 'border-accent-lime ring-1 ring-accent-lime/30 shadow-[0_4px_20px_rgba(194,239,78,0.1)]'
											: 'border-hairline-violet/50'
									}`}
								>
									<div>
										<div className="flex items-center justify-between mb-6">
											<span
												className={`text-xs font-bold uppercase tracking-[1px] px-3 py-1 rounded-full ${
													isSelected
														? 'text-ink-deep bg-accent-lime'
											...
											<span
												className={`font-display text-2xl font-bold transition-colors ${
													isSelected
														? 'text-accent-lime'
														: 'text-on-dark-faint group-hover:text-accent-lime/20'
												}`}
											>
												{p.badge}
											</span>
					...
					>
						...
						<div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6 pb-4 border-b border-hairline-violet/30">
							<div>
								...
								<h4 className="font-display text-lg font-bold text-on-primary mt-1">
									Sample Checklist: {TIMELINE_PHASES[activeTimeline].title} (
									{TIMELINE_PHASES[activeTimeline].phase})
								</h4>
							</div>
						...
						<div className="grid gap-4 sm:grid-cols-2">
							{TIMELINE_PHASES[activeTimeline].tasks.map((task, i) => (
								<div
									key={i}
									className="flex gap-3 items-start p-4 bg-surface-canvas-dark/40 border border-hairline-violet/20 rounded-lg hover:border-hairline-violet/60 transition-colors"
								>
				...
				<div className="text-center mt-12 text-xs text-on-dark-muted">
					* Cat decompression cycles vary. The 3-3-3 timeline acts as a general
					behavioral baseline.
				</div>
			</div>
		</section>
