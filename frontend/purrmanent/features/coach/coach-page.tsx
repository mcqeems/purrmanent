...
export function CoachPage() {
  ...
  const firstName = session?.user?.name ? session.user.name.split(' ')[0] : 'Mustaqim';
    ...
    <div className="relative flex flex-col flex-1 h-full w-full bg-transparent">
      ...
      <div className="relative z-10 flex flex-col flex-1 h-full select-none">
        ...
        <div className={cn(
          ...
          hasMessages ? "flex-1 overflow-y-auto p-4 space-y-3" : "h-0 overflow-hidden"
        )} ref={scrollRef}>
          {hasMessages && messages.map((m) => {
            if (m.role === 'assistant' && !m.content && !m.pending && !(m.sources && m.sources.length)) {
              return null;
            }
            return (
              <div
        ...
        <div className={cn(
          ...
          hasMessages ? "pt-2 pb-3 px-4 border-t border-slate-100 bg-white" : "flex-1 justify-center py-10"
        )}>
          {!hasMessages && (
            <motion.h2 
          ...
          <div className="w-full max-w-2xl px-4">
            <InputPill value={input} onChange={setInput} onSubmit={submit} streaming={streaming} />
          </div>
